#include <bits/stdc++.h>
using namespace std;

const int MAXN = 1e5 + 9;
const long long NO_LAZY = -1; // means no pending assignment

long long a[MAXN], t[MAXN * 4], lazy[MAXN * 4];

void push(int n, int b, int e) {
    if (lazy[n] == NO_LAZY) return;
    t[n] = lazy[n]; // assign the value
    if (b != e) {
        int l = n * 2, r = n * 2 + 1;
        lazy[l] = lazy[n];
        lazy[r] = lazy[n];
    }
    lazy[n] = NO_LAZY;
}

void build(int n, int b, int e) {
    lazy[n] = NO_LAZY;
    if (b == e) {
        t[n] = a[b];
        return;
    }
    int mid = (b + e) / 2;
    build(n * 2, b, mid);
    build(n * 2 + 1, mid + 1, e);
    t[n] = 0;
}

void update(int n, int b, int e, int i, int j, long long v) {
    push(n, b, e);
    if (e < i || j < b) return;
    if (i <= b && e <= j) {
        lazy[n] = v;
        push(n, b, e);
        return;
    }
    int mid = (b + e) / 2;
    update(n * 2, b, mid, i, j, v);
    update(n * 2 + 1, mid + 1, e, i, j, v);
}

long long query(int n, int b, int e, int idx) {
    push(n, b, e);
    if (b == e) return t[n];
    int mid = (b + e) / 2;
    if (idx <= mid) return query(n * 2, b, mid, idx);
    else return query(n * 2 + 1, mid + 1, e, idx);
}

int main() {
    ios::sync_with_stdio(false);
    cin.tie(nullptr);

    int n, m;
    cin >> n >> m;
    build(1, 0, n - 1);

    while (m--) {
        int type;
        cin >> type;
        if (type == 1) {
            int l, r;
            long long v;
            cin >> l >> r >> v;
            update(1, 0, n - 1, l, r - 1, v);
        } else {
            int i;
            cin >> i;
            cout << query(1, 0, n - 1, i) << "\n";
        }
    }
    return 0;
}
